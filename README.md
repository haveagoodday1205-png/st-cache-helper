...
patched      system／动态上下文改变，在尾部追加最小文本补丁
revised      旧对话改变，在尾部追加权威修订
...
- system 修订不再复制完整 system：按块匹配后对大文本生成行／字符级最小补丁，未变化的预设、角色卡和世界书不会再次进入缓存写入尾部。
- 动态上下文从按数组下标整块替换改为序列补丁；插入、删除和顺序移动会保留相同块，块内只有少量状态或时间变化时只发送变化区间。
- 新增自适应 epoch 重建：单次补丁接近完整正文或累计修订明显膨胀时，允许一次干净冷写，避免请求上下文无限增长。
- 全局日志协议升级为 v2 并自动清理旧 v1 会话日志；升级后的首次请求会正常冷写一次，之后才开始读取新前缀。
- 真实 Request Monitor 回放中，首轮 `45,207` 字符动态上下文在下一轮只产生 `1,004` 字符补丁；包含新增对话在内的总追加量为 `3,112` 字符。另一组 system 改写只产生 `555` 字符补丁。
